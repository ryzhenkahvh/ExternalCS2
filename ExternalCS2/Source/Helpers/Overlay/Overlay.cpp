...
// Говнокод
void Overlay::DrawCrosshair(long centerX, long centerY, DWORD color)
{
	const long crosshairSize = 30;    // Ещё больше увеличиваем размер
	const long crosshairGap = 8;      // Увеличиваем промежуток
	const long thickness = 3;         // Увеличиваем толщину

	// Горизонтальные линии
	DrawFilledRect(
		centerX - crosshairSize - crosshairGap,
		centerY - thickness,
		centerX - crosshairGap,
		centerY + thickness,
		0xFFFF0000  // Используем красный цвет напрямую
	);
	...
	DrawFilledRect(
		centerX - thickness,
		centerY - crosshairSize - crosshairGap,
		centerX + thickness,
